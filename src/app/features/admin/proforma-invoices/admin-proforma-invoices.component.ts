...
export class AdminProformaInvoicesComponent implements OnInit {
  ...
  convertToInvoice(invoice: SalesInvoice): void {
    if (!confirm(`Convert proforma invoice ${invoice.invoiceNumber} to tax invoice? This action cannot be undone.`)) {
      return;
    }

    this.invoicesService.convertProformaToInvoice(invoice.id).subscribe({
      next: () => {
        this.snackBar.open('Proforma Invoice converted to Tax Invoice successfully', 'Close', {
          duration: 3000,
        });
        this.loadProformaInvoices();
      },
